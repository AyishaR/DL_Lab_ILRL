...
def split_proportionally(X, y):
    X_train, X_valid, y_train, y_valid = train_test_split(X, y, test_size=0.1, random_state=None, stratify=y)
...
def train_model(
    ...
    X, y = preprocessing(
        X, y, history_length=history_length)

    for epoch in range(epochs):
        
        X_train, y_train, X_valid, y_valid = get_train_test(X, y, custom_sizes=custom_sizes)
...
if __name__ == "__main__":
    ...
    for history_length in [0,1,3,5]:
        ...
        train_model(X, y, device, n_minibatches=300, batch_size=64, lr=1e-4, history_length=history_length, agent_path=agent_path, epochs = 4,
